...
public class InsertImpl implements Insert {
    ...
    private Integer insert(int classId, List<String> columns, Tuple tuple, int l, int[] index) throws TMDBException {
        ...
        ArrayList<Integer> tJoinDeputyIdList = tJoinDeputySize(classId);
        //如果list非空就需要进一步判断
        if(tJoinDeputyIdList.size()!=0){
            ...
            //对代理类idlist进行遍历
            for (int i = 0; i < tJoinDeputyIdList.size(); i++) {
                //得到当前代理类的id
                int deputyId = tJoinDeputyIdList.get(i);
                //由于基于轨迹相似度的代理类需要两个源类，需要拿到另一个源类，这里使用getAnotherDeputy方法拿到
                int anotherDeputy = getAnotherDeputy(deputyId, classId);
                //通过select的getTable（classid）方法拿到另一个源类的所有tuple
                TupleList anotherTuples = select.getTable(anotherDeputy);
                //遍历另一个源类的所有tuple
                for (int j = 0; j < anotherTuples.tuplelist.size(); j++) {
                    //拿到另一个源类的当前tuple
                    Tuple tuple2 = anotherTuples.tuplelist.get(j);
